...
def create_page_3():
    ...
    title_3_label = Label(second_frame, text=info.peaks_title.get(info.selected_peaks[2]), font=('roboto', 20, 'bold'),
                          background="#909C74")
    ...
    img_1_holder.grid(row=1, rowspan=2, column=2, columnspan=2, padx=8, pady=5)
    ...
    img_2_holder.grid(row=1, rowspan=2, column=6, columnspan=1, padx=8, pady=5)
    ...
    img_3_holder.grid(row=1, rowspan=2, column=10, columnspan=1, padx=8, pady=5)
    ...
    # Get info for the third peak from my web scraper
    info_dictionary_3 = web_scraper.info_box_scrape(info.peak_bagger_urls.get(info.selected_peaks[2]))
    print(info_dictionary_3)
    ...
    prominence_3 = Label(info_box_3, text="Prominence: " + info_dictionary_3.get('Prominence'),
                         bg="#D3D3D3")
    prominence_3.grid(row=2, column=0, sticky=W)

    elevation_3 = Label(info_box_3, text="Elevation: " + info_dictionary_3.get('Elevation'), bg="#D3D3D3")
    elevation_3.grid(row=1, column=0, sticky=W)

    isolation_3 = Label(info_box_3, text="Isolation: " + info_dictionary_3.get('Isolation'), bg="#D3D3D3")
    isolation_3.grid(row=3, column=0, sticky=W)

    state_3 = Label(info_box_3, text="State: " + info_dictionary_3.get('State'), bg="#D3D3D3")
    state_3.grid(row=4, column=0, sticky=W)

    coords_3 = Label(info_box_3, text="Coordinates: " + info_dictionary_3.get('Coordinates'), bg="#D3D3D3")
    ...
    link_3 = Label(second_frame, text=info.peaks_title.get(info.selected_peaks[2]) + ' on peakbagger.com',
                   font=('Helveticabold', 12), fg="blue", cursor="hand2", bg="#909C74")

    link_3.grid(row=5, column=10)
    link_3.bind("<Button-1>", lambda e: callback(info.peak_bagger_urls.get(info.selected_peaks[2])))
    ...
    # Image 3
    all_trail_img_3 = img_from_url(info.all_trails_maps.get(info.selected_peaks[2]))
